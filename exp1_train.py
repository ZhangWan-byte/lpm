...
def load_node_features(graph_dir: str, standardize: bool = True) -> np.ndarray:
    """
    Load node features for the graph from the saved nodes file.
    Priority:
      1) 'node_features' array if present
      2) else derive from latent 'positions' using a simple nonlinear emission f
    """
    # find *_nodes.npz
    ...
    if "node_features" in npz.files:
        ...
        # Use positions to synthesize observable features via a nonlinear map f
        # so that the encoder approximates f^{-1}.
        X = np.array(npz["positions"], dtype=np.float32)  # [N, d]
        # Build a small feature basis from positions:
        # [x, x^2, sin(wx), cos(wx)] with a few random Fourier components.
        N, d = X.shape
        rng = np.random.default_rng(7)
        m = min(4 * d, 32)  # number of random frequencies
...
def step_elbo(
    ...
    loss, stats = model.elbo(A_norm, pos, neg, feats=feats)
    return loss, stats


def parse_args():
    ap = argparse.ArgumentParser(description="Train RG-VAE on a SINGLE generated RGM dataset (Experiment 1)")
    ap.add_argument("--data_root", required=True, help="Root folder containing all generated sets (e.g., D:/rebuttal2025/out)")
    ap.add_argument("--rgm_type", required=True, help="Which dataset to train on (e.g., A1, A2, B2, ...). Matches subfolder name.")
    ap.add_argument("--models_dir", default="models/checkpoints", help="Where to save checkpoints (default ./models)")
    ...
    ap.add_argument("--decoder_kwargs", default="{}",
                    help='JSON dict of kwargs for the decoder (e.g., \'{"num_features":512,"lengthscale":2.0}\')')
...
def main():
    ...
    # Load or synthesize node features (observable), standardize
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        tr_loss += float(loss.item()); tr_recon += stats["recon"]; tr_kl += stats["kl"]
        ...
        print(
            f"Epoch {epoch:03d} | "
            f"train: total={tr_loss:.4f}, recon={tr_recon:.4f}, KL={tr_kl:.4f}  ||  "
            f"val: total={float(val_loss.item()):.4f}, recon={val_stats['recon']:.4f}, KL={val_stats['kl']:.4f}"
        )
